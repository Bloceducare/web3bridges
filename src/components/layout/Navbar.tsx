...
export function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-page/95 backdrop-blur-sm border-b border-ink/5">
      ...
      {/* Mobile menu */}
      {mobileOpen && (
        <div className="lg:hidden bg-card border-t border-ink/5">
          <div className="container-wide py-4 space-y-1">
            {[{ label: 'Home', href: '/' }, ...navLinks.slice(1)].map(link => (
              <Link key={link.href} href={link.href} className="block px-3 py-2.5 text-sm text-ink/60 hover:text-ink hover:bg-ink/5 rounded-lg transition-colors">
                {link.label}
              </Link>
            ))}
            <div className="pt-2 border-t border-ink/5 space-y-1">
              ...
              <p className="px-3 pt-2 text-[10px] font-semibold text-ink/30 uppercase tracking-widest">Sprints</p>
              {sprints.map(s => (
